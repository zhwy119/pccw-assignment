...
public class UserController {
     ...
     *
     * @param ids The IDs of the user to deactivate.
     * @return RestResult indicating the result of the operation.
     */
    @DeleteMapping
    @ApiOperation(value = "Delete users", notes = "Deactivates existing users")
    public RestResult deactivateUsers(@RequestBody List<Long> ids) {
        if (ids == null || ids.isEmpty()) {
            return RestResult.fail(ResultCodes.FAILURE, "User IDs are required");
        }
        try {
            ...
            log.error("error happened while deleting users", e);
            return RestResult.error();
        }
        return RestResult.success();
    }
